...
trait SwingApi {
  ...
  implicit class TextFieldOps(field: TextField) {
    ...
    /** Returns a stream of text field values entered in the given text field.
      ...
      */
    def textValues: Observable[String] = Observable.create { obj => {
      val rx: Reaction = { case ValueChanged(tField) => obj.onNext(tField.text) }
      field subscribe rx
  ...
  implicit class ButtonOps(button: Button) {
     ...
     */
    def clicks: Observable[Button] = Observable.create { obj => {
      val rx: Reaction = { case ButtonClicked(btn) => obj.onNext(btn) }
      button subscribe rx
